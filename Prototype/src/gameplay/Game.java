...
/**
 * <p>
 * The <code>Game</code> class is contains the main game loop which does all the
 * drawing, as well as some gameplay logic.
 * </p>
 * 
...
public class Game extends JPanel implements ActionListener, Serializable {
	 ...
	 *            specification
	 */
	public Game(Grid grid, Level level, Enemy enemy, Brick brick,
			Concrete concrete, PowerUps powerups, ExitWay exitway, Bomb bomb,
	...
	public void paint(Graphics g) {
		...
		if (GameState.getState() == State.PAUSE) {
			...
			for (int i = leftMostVisibleTile; i <= rightMostVisibleTile; i++) {
				for (int j = 0; j < Bomberman.HEIGHT; j++) {
					...
					case ONEAL:
						...
						continue;
					case DOLL:
						g2d.drawImage(enemy.getImageDoll(), Bomberman.TILE_SIZE
								* (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
					case MINVO:
					...
					case KONDORIA:
						g2d.drawImage(
								enemy.getImageKondoria(),
								Bomberman.TILE_SIZE * (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
					case KONDORIAANDBRICK:
						g2d.drawImage(
								enemy.getImageKondoriaAndBrick(),
								Bomberman.TILE_SIZE * (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
					case OVAPI:
					...
					case OVAPIANDBRICK:
						g2d.drawImage(
								enemy.getImageOvapiAndBrick(),
								Bomberman.TILE_SIZE * (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
					case PASS:
						g2d.drawImage(enemy.getImagePass(), Bomberman.TILE_SIZE
								* (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
					case PONTAN:
						g2d.drawImage(
								enemy.getImagePontan(),
								Bomberman.TILE_SIZE * (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
					case PONTANANDBRICK:
						g2d.drawImage(
								enemy.getImagePontanAndBrick(),
								Bomberman.TILE_SIZE * (i - leftMostVisibleTile),
								Bomberman.TILE_SIZE * j, this);
						continue;
			...
			if (count == 3) {
				...
				enemy.move(player.getX() / Bomberman.TILE_SIZE, player.getY()
						/ Bomberman.TILE_SIZE);
				count = 0;
			}
			count++;
	...
	private void checkIfAllEnemiesDead() {
		if (enemy.getEnemyCount() == Bomb.getNumberOfEnemiesKilled()) {
			...
			System.out.println("Level over, head to the exitway!");
		}

	}

	/**
	 * Method which computes the appropriate range of tiles on the grid to be
	...
	/**
	 * Method that verifies if the player is still alive.
	 */
