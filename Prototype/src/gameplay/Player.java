...
/**
 * <p>
 * The <code>Player</code> class takes all user input and deals will all the
 * logic related to the basic functions of bomberman, including movement,
 * collisions, death, powerups, bombs, and pausing.
 * </p>
 * 
...
public class Player implements Serializable {
	...
	/**
	 * <p>
	 * Constructor that initializes a <code>Player</code> object which
	 * initializes the user's starting position on the map, sets its position on
	 * the frame, initializes all bomb attributes to their basic values, sets
	 * the score for the session to zero, initializes movement speed and loads
	 * the player image. Additionally, the required game objects are
	 * initialized. A player utilizes a Grid, Enemy, and PowerUps object.
	 * </p>
	 * 
	 * A player object deals with the basic player movement and collisions,
	 * powerup logic, and places bomb accordingly. Additionally, all user input
	 * is taken through the player class.
	 * 
	 * @param grid
	...
	/**
	 * <p>
	 * Move method called at every user input. Based on the key pressed, it
	 * updates the players position on the grid accordingly. This method deals
	 * with collisions, powerups, and player death.
	 * </p>
	 * 
	 * <p>
	 * Move only occurs when the countForMovementSpeed is equal to the actual
	 * movement speed, which is defaulted to 2. The count is updated on each
	 * call, and is reset when the move is executed. When the player finds the
	 * movement speed power up, the movementSpeed is set to 1, and move
		...
		if (countForMovementSpeed == movementSpeed) {
			if (dx != 0) {
				...
				if (grid.getContents(posX + (dx / 25), posY) == Tile.EMPTY
						...
						|| (grid.getContents(posX + (dx / 25), posY) == Tile.BOMB && PowerUps
								.getBombpass() == true)
						|| (grid.getContents(posX + (dx / 25), posY) == Tile.BRICK && PowerUps
								.getWallpass() == true)
						|| (grid.getContents(posX + (dx / 25), posY) == Tile.BRICKANDEXITWAY && PowerUps
								.getWallpass() == true)
						|| (grid.getContents(posX + (dx / 25), posY) == Tile.BRICKANDPOWERUPS && PowerUps
								.getWallpass() == true)) {
					...
					} else if (grid.getContents(posX, posY) == Tile.BOMB
							&& PowerUps.getBombpass() == true) {
						grid.setContents(posX, posY, Tile.PLAYERANDBOMB);
					} else if (grid.getContents(posX, posY) == Tile.BRICK
							&& PowerUps.getWallpass() == true) {
						grid.setContents(posX, posY, Tile.PLAYERANDBRICK);
					} else if (grid.getContents(posX, posY) == Tile.BRICKANDEXITWAY
							&& PowerUps.getWallpass() == true) {
						grid.setContents(posX, posY,
								Tile.PLAYERANDBRICKANDEXITWAY);
					} else if (grid.getContents(posX, posY) == Tile.BRICKANDPOWERUPS
							&& PowerUps.getWallpass() == true) {
						grid.setContents(posX, posY,
								Tile.PLAYERANDBRICKANDPOWERUPS);
			...
			if (dy != 0) {
				...
				if (grid.getContents(posX, posY + (dy / 25)) == Tile.EMPTY
						...
						|| (grid.getContents(posX, posY + (dy / 25)) == Tile.BOMB && PowerUps
								.getBombpass() == true)
						|| (grid.getContents(posX, posY + (dy / 25)) == Tile.BRICK && PowerUps
								.getWallpass() == true)
						|| (grid.getContents(posX, posY + (dy / 25)) == Tile.BRICKANDEXITWAY && PowerUps
								.getWallpass() == true)
						|| (grid.getContents(posX, posY + (dy / 25)) == Tile.BRICKANDPOWERUPS && PowerUps
								.getWallpass() == true)) {
	...
	/**
	 * <p>
	 * This method performs a check on if the following keys are pressed: up,
	 * down, right, left, b, space, and x. Based on the key, It then processes
	 * the required logic. .
	 * </p>
	 * 
	 */
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_X:
			if ((GameState.getState() == State.RUNNING || GameState.getState() == State.RUNNINGANDLEVELOVER)) {
				if (grid.getContents(posX, posY) != Tile.PLAYERANDBOMB
						&& (grid.getContents(posX, posY) != Tile.PLAYERANDEXITWAY)) {
	 ...
	 * 
	 * @return the <code>int</code> corresponding to the player's position
	 *         relative to the frame's initial point along the 'x' axis.
	 */
	 ...
	 * 
	 * @param xNew
	 *            The new position of the player along the x axis.
	 */
	 ...
	 * 
	 * @return the <code>int</code> corresponding to the player's position
	 *         relative to the frame's initial point along the y axis.
	 */
	 ...
	 * 
	 * @param yNew
	 *            The new position of the player along the y axis.
	 */
	...
	/**
	 * Method used to load the player's image. This corresponds to the bomberman
	 * icon.
	 */
	...
	/**
	 * <p>
	 * Returns the bombNumber which keeps track of the bombs placed on the map:
	 * required for the detonate powerup logic.
	 * </p>
	 * This method allows you to determine the bomb number.
	 * <p>
	 * Note: bombNumber is reset when there are no bombs present on the grid.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Required for detonate powerup logic.
	 * </p>
	 * <p>
	 * Sets the bombNumber which keeps track of the bombs placed on the map.
	 * </p>
	 * This method allows you to set the bomb number. This is primarily used in
	 * the bomb class: after the explode() method is called, it is incremented.
	 * <p>
	 * Note: bombNumber is reset when there are no bombs present on the grid.
	 * </p>
	 * 
	 * @param bombNumber
	 *            The number corresponding to the bomb placed on the grid. The
	 *            oldest bomb should be set to 10.
	 */
	 ...
	 * Required for detonate powerup logic.
	 * </p>
	 * Gets the counter for the integer which keeps track of the bomb number,
	 * and is passed to the new bomb object and decremented. This is cross
	 * checked with bombNumber; if they match, the bomb explodes.
	 * <p>
	 * </p>
	 * 
	 * @return the <code>int</code> corresponding to the currentBombCounter
	 *         attribute.
	 */
	 ...
	 * Required for detonate powerup logic.
	 * </p>
	 * Sets the counter for the integer which keeps track of the bomb number,
	 * and is passed to the new bomb object and decremented. This is cross
	 * checked with bombNumber; if they match, the bomb explodes.
	 * 
	 * @param currentBombCounter
	 *            The number corresponding to the bomb number.
	 */
	...
	/**
	 * Sets the player's score. Required when enemies are killed in the bomb
	 * class.
	 * 
	 * @param score
	 *            The Player's score.
	 ...
	 * 
	 * @return the <code>int</code> corresponding the Player's number of lives
	 *         left.
	 */
	 ...
	 * 
	 * @param range
	 *            The range of the bomb explosion.
	 */
	 ...
	 * 
	 * @return the <code>int</code> getNumberOfBombs representing the number of
	 *         bombs that can be placed at one time by the Player.
	 */
	 ...
	 * 
	 * @return the <code>int</code> corresponding to the number of bombs that
	 *         are currently on the grid.
	 */
	 ...
	 * 
	 * @param bombsOnGround
	 *            The new integer representing the number of bombs present on
	 *            the grid.
	 */
	 ...
	 * 
	 * @return the <code>boolean</code> representing whether or not C has been
	 *         pressed.
	 */
	 ...
	 * 
	 * @param detonatePressed
	 *            The boolean representing whether or not C has been pressed.
	 */
	 ...
	 * 
	 * @return the <code>boolean</code> representing whether or not the player
	 *         has the detonate powerup.
	 */
	...
	/**
	 * Method to change the Player's movement speed. As it can only be 1 or 2,
	 * (1 being the faster of the two), it only needs to be toggled.
	 */
