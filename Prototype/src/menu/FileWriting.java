...
/**
 * FilWriting implements all file writing to userInfo.csv,userStatistics.csv and
 * saving a game to whatever file the user selects. All userInfo after being
 * pulled from the files is stored in ArrayList<CSVRecord> to compare with input
...
public class FileWriting implements Serializable {
	 ...
	 * checks if there is a match. Then lets the user log in or rejects username
	...
	/**
	 * checks if the username is availible and then loops through file checking
	 * if username is taken if yes returna a warning to the user and does let
	 * the user create an account
	 * 
	 * @param username
	...
	/**
	 * first checks if the file exists then opens it and reads all the data
	 * using apache csv parser into an arraylist with usernames and passwords
	 */
	...
	/**
	 * Wites the realname,username,password to the file at a new line this
	 * method is only called after having checked that the combination is a
	 * valid one.
	 * 
	 * @param realName
	 ...
	 *            String of retyping the user's password for verification
	 *            purposes.
	 * @throws IOException
	 */
	...
	/**
	 * overwrites to file when the user tries to modify his account assuming
	 * correct input of realname and password The user is not allowed to modify
	 * his username
	 * 
	 * @param realName
	 ...
	 * @throws IOException
	 */
	public void overwriteToFileString(String realName, String username,
			String password, String retypePassword) throws IOException {
		String fileName = System.getProperty("user.dir") + "//" + "src" + "//"
				+ "menu" + "//" + ("userInfo.csv");
		...
		for (int i = 0; i < (loginInfo).size(); i = i + 1) {
			...
			if (loginInfo.get(i).get(usernamePosition)
					.equals(PlayerInfo.getUsername())) {
				writer.printRecord(realName, username, password);
	...
	/**
	 * this method opens the statistics file for reading and then stores the
	 * data in an arraylist called user statistiscs.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * This writes to statistics the username, score and level unlocked. It will
	 * overwrite should the user already have a stored data point.
	 * 
	 * @param username
	 ...
	 * @throws IOException
	 */
	public void writeToStatisticsFile(String username, int score,
			int levelUnlocked) throws IOException {
	...
	/**
	 * Takes all the input neccesary to serialize the game and then save it in a
	 * file of the users choice. All of these objects are serializable
	 * 
	 * @param fileName
	 ...
	 * @param powerUps
	 *            Object of type PowerUps stores the all the powerups
	 *            information.
	 * @param exitWays
	 ...
	 * @param flamePass
	 *            Boolean of type flamePass does the player have the flamePass
	 *            powerup.
	 * @param bombPass
	 *            Boolean of type bombPass does the player have the flamePass
	 *            powerup.
	 * @param wallPass
	 *            Boolean of type wallPass does the player have the flamePass
	 *            powerup.
	 * @param detonate
	 *            Boolean of type detonate does the player have the flamePass
	 *            powerup.
	 * @throws IOException
	 */

	public void saveGame(String fileName, Level level, Enemy enemy,
			Player player, Concrete concrete, Brick brick, PowerUps powerUps,
			ExitWay exitWays, Bomb bomb, Grid grid, int playerScore,
			int currentLevel, int currentLives, int timer, boolean flamePass,
		...
		if (savingFile.exists()) {
			...
			JOptionPane.showMessageDialog(null,
					"would you like to overwrite file", "error",
	...
	/**
	 * Loads all the objects stored in the filename and initializes them and
	 * they are called to loadgame by the getters below.
	 * 
	 * @param fileName
	 ...
	 * @throws IOException
	 */
	public void loadGame(String fileName) throws ClassNotFoundException,
			IOException {
		...
		if (loadingFile.exists()) {
			...
			JOptionPane.showMessageDialog(null, "file doesnt exist", "error",
					JOptionPane.INFORMATION_MESSAGE);
			return;
		}
